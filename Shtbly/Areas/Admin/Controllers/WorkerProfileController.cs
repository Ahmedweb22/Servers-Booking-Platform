...
namespace Shtbly.Areas.Admin.Controllers
{
    ...
    public class WorkerProfileController : Controller
    {
        ...
        {
            var existingWorkerUserIds = (await _workerProfileRepo.GetAsync(tracking: false))
                .Select(w => w.UserId)
                .ToHashSet();
            ...
            var vm = new WorkerProfilesVM
            {
                Users = (await _userRepo.GetAsync(tracking: false))
                    .Where(u => !existingWorkerUserIds.Contains(u.Id))
                    .ToList()
            };
            return View(vm);
        }
        ...
        public async Task<IActionResult> Create(WorkerProfilesVM model)
        {
            ...
            var existingProfile = await _workerProfileRepo.GetOneAsync(w => w.UserId == model.UserId);
            if (existingProfile != null)
            {
                model.Users = await _userRepo.GetAsync(tracking: false);
                TempData["error-notification"] = "This user already has a worker profile.";
                return View(model);
            }
            ...
            if (!string.IsNullOrWhiteSpace(model.UserId) && model.CVFile is not null)
            {
